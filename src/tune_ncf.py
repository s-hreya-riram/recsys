'''
Hyperparameter tuning for NCF model using val set.
Run: python src/tune_ncf.py --dataset movielens
'''
...
def tune(cfg):
    ...
    for combo in product(*values):
        ...
        row = {
            'emb_dim':    params['emb_dim'],
            'mlp_layers': str(params['mlp_layers']),
            'lr':         params['lr'],
            'n_neg':      params['n_neg'],
            **metrics
        }
        ...
        print(f"  emb={params['emb_dim']} layers={params['mlp_layers']} "
    ...
    out = BASE_DIR / 'results' / cfg.name / 'ncf_tuning.csv'
